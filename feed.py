...
def get_feed_item(curr_item):
    '''
    TODO: limit text to 8000 characters
    '''
    posts = parse_rss_feed(RSS_PATH).entries
    post = posts[curr_item]
    logging.info('curr_item is %s', curr_item)
    if curr_item != len(posts):
        entry = {}
        entry['title'] = post.title
        entry['link'] = post.link
        entry['description'] = remove_url(post.description)
...
def get_post(post_id, posts):

    return posts[post_id]

def remove_url(text):
    '''
    removes URL from text based on regex pattern:
